...
async function testDatabase() {
	...
	try {
		...
		users.rows.forEach((user: User) => {
			console.log(
				`   - ${user.name} (${user.email}) - Plan: ${user.plan_tier}`,
			);
		});
		...
		if (user) {
			...
			if (websites.length > 0) {
				...
				const scan = await db.createScanResult(
					websites[0].id,
					'manual',
					user.id,
				);
				...
				if (alertPrefs) {
					...
					console.log(
						`   Alert on new broken: ${alertPrefs.alert_on_new_broken}`,
					);
					console.log(`   Alert on fixed: ${alertPrefs.alert_on_fixed}`);
					console.log(
						`   Recipients: ${alertPrefs.email_recipients.join(', ')}`,
					);
				}
			}
		}
